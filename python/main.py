...
class SqlDb:
	...
	def get(self):
		self.refresh()
		return list(self.cur.execute(f"SELECT * FROM {self.tab}").fetchall()[0])
	def refresh(self):
		self.cnn = sqlite3.connect(self.dat)
		self.cur = self.cnn.cursor()
	def update(self, dtnm, dlis):
		self.refresh()
		self.cur.execute(f"UPDATE {self.tab} SET 	{dtnm[0]} = {dlis[0]}, {dtnm[1]} = {dlis[1]} WHERE rowid = 1")
		self.cnn.commit()
class B1(Button):
...
class main(App):
	...
	def logics(self, *args):
		...
		if self.b2_2.text == "play":
			self.now, self.rt, self.event1, self.b2_2.text, self.buttons[self.switch[0]].disabled = time.time(), [[self.rt[0][0], self.rt[0][1]], None], Clock.schedule_interval(self.update, .005), "pause", False
			...
			self.b2_2.text, self.btn, self.switch[1] = "play", ["play", "restart"], 0
		elif self.b2_2.text == "restart":
			self.btn, self.switch[1], self.tcc, self.rt, self.switch[0] = ["play", "restart", "setting"], 1, [0, 0], [[0, 0], 300], 0
			self.b_setting2(True, True)
			self.updates()
			...
			self.sql.update(["data1", "data2"], self.timer)
	...
	def update(self, *args):
		if self.rt[1] == None:
			...
			self.labels[not self.switch[0]].text = self.TC((self.timer[not self.switch[0]]-self.tcc[not self.switch[0]])+self.rt[0][not self.switch[0]])
			self.rt[0][self.switch[0]], self.rt[1] = 0, self.switch[0]
			Time, self.now = (self.timer[self.switch[0]]-self.tcc[self.switch[0]])+self.rt[0][self.switch[0]], time.time()
		...
		if Time <= 0:
			...
			self.b_setting2(True, True)
			self.b2_2.text, self.btn, self.switch[1] = "restart", ["restart"], 0
			self.b_setting1(False, False)
			Clock.unschedule(self.event1)
